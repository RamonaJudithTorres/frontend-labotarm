...
export function getAllEstudios(page_number, page_size, nombre){
    ...
    try{
        const response= await axios.get(`https://s8o2l4qbll.execute-api.us-east-1.amazonaws.com/dev/api/servicios/estudios/all?page_number=${page_number}&page_size=${page_size}&nombre=${nombre}`);
...
export function createEstudio ({nombre, clave, descripcion}){
    return async dispatch=>{
        dispatch(estudioSlice.actions.startLoading);
        try{
            const response = await axios.post('https://s8o2l4qbll.execute-api.us-east-1.amazonaws.com/dev/api/servicios/estudios/create/',{
                nombre,
                clave,
                descripcion,     
             })
